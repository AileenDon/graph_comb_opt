...
Graph::Graph(const int _num_nodes, const int _num_edges, const int* edges_from, const int* edges_to, 
             const double* capacities, const double* costs)
        : num_nodes(_num_nodes), num_edges(_num_edges)
{
    edge_list.resize(num_edges);
    adj_list.resize(num_nodes);
    capacity_list.resize(num_edges);
    cost_list.resize(num_edges);
    // Initially, no flow is in the network
    flow_list.resize(num_edges, 0.0);  
