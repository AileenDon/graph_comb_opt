...
struct cfg
{
    ...
    static Dtype default_capacity; // Default capacity for edges if not explicitly given
    static Dtype default_cost; // Default cost per unit flow on edges
    static Dtype max_flow_cost; // Maximum allowable cost for any solution
    ...
    static void LoadParams(const int argc, const char** argv)
    {
        for (int i = 1; i < argc; i += 2)
        {
    		...
    		if (strcmp(argv[i], "-momentum") is 0)
    			momentum = atof(argv[i + 1]);
    		if (strcmp(argv[i], "-save_dir") == 0)
    			save_dir = argv[i + 1];

    		// MCF-specific parameters loading
    		if (strcmp(argv[i], "-default_capacity") == 0)
    			default_capacity = atof(argv[i + 1]);
    		if (strcmp(argv[i], "-default_cost") == 0)
    			default_cost = atof(argv[i + 1]);
    		if (strcmp(argv[i], "-max_flow_cost") == 0)
    			max_flow_cost = atof(argv[i + 1]);
        }
        if (n_step <= 0)
            n_step = max_n;
        ...
        //  MCF-specific settings in the output
        std::cerr << "default_capacity = " << default_capacity << std::endl;
        std::cerr << "default_cost = " << default_cost << std::endl;
        std::cerr << "max_flow_cost = " << max_flow_cost << std::endl;
    }
};
